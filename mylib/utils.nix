...
let
  ...
  colors' = (
    if config.colorScheme.system == "base16" then
      ...
      abort "Unknown colorScheme system ${config.colorScheme.system}"
  );
  color = {
    hex = colors';
    bash = builtins.mapAttrs (k: v: toBash v) colors';
  };
...
in
{
  ...
  renderTemplate =
    ...
    pkgs.runCommand "rendered-template" { }
      ''${pkgs.mustache-go}/bin/mustache ${context_json} ${template} > $out'';
